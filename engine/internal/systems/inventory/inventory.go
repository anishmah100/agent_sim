	...
	return manifest.SystemDeclaration{
		...
		Verbs: []manifest.VerbDeclaration{
			{Verb: "pickup", Description: "Pick up an adjacent item.",
				...
				RejectionReasons: []string{"bad_params", "not_an_item", "unknown_target", "target_too_far", "inventory_full", "money_service_missing"},
			...
			{Verb: "give", Description: "Give an inventory item to an adjacent target.",
				...
				RejectionReasons: []string{"bad_params", "unknown_target", "not_a_target", "self_target", "target_too_far", "not_in_inventory", "inventory_full"},
			...
			{Verb: "eat", Description: "Consume a food item from inventory; subtracts its satiety from hunger (clamped at 0).",
				ParamsSchema:     json.RawMessage(`{"type":"object","properties":{"item":{"type":"string"}},"required":["item"]}`),
				RejectionReasons: []string{"bad_params", "not_in_inventory", "not_food"},
				EmitsEvents:      []string{"AteFood"},
			},
